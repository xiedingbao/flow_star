...
/*add by Dingbao Xie, reachability analysis along certain path */
bool HybridSystem::reach_path(list<list<TaylorModelVec> > & resultsCompo, list<list<vector<Interval> > > & domains, list<int> & modeIDs, list<TreeNode *> & traceNodes,
		TreeNode * & traceTree, const vector<int> & integrationSchemes, const double step, const double miniStep,
		const double time, const int orderType, const vector<int> & orders, const vector<int> & maxOrders, const int globalMaxOrder,
		const bool bAdaptiveSteps, const bool bAdaptiveOrders,vector<int> path, const int precondition, const vector<Interval> & estimation,
		const vector<vector<int> > & aggregType, const vector<vector<vector<RowVector> > > aggregationTemplate_candidates, const vector<RowVector> default_aggregation_template,
		const vector<vector<Matrix> > & weightTab, const vector<vector<vector<bool> > > & linear_auto, const vector<vector<vector<RowVector> > > & template_auto,
		const bool bPrint, const vector<string> & stateVarNames, const vector<string> & modeNames, const vector<string> & tmVarNames) const
{
	...
	else if(initialMode!=path[0]){
		printf("The path should start from the initial mode!\n");
		exit(1);
	}
	...
	for(; modeQueue.size() != 0;)
	{
		...
		if(!bvalid)
		{
			fprintf(stderr,"The computation result is not valid\n");
			exit(1);
		}
		vector<vector<Interval> > step_exp_tables;
		vector<Interval> step_exp_table;
		...
		for(int i=0; i<transitions[initMode].size(); ++i)
		{
			//printf("current mode:%s,jump:%i\n", modeNames[initMode].c_str(),jumpsExecuted);
			if(transitions[initMode][i].targetID!=path[jumpsExecuted+1])
				continue;
			vector<TaylorModelVec> intersection_flowpipes;
			vector<vector<Interval> > intersection_domains;
			list<TaylorModelVec>::iterator tmvIter = mode_flowpipes.begin();
...
/* DFS based reachability analysis, Dingbao Xie  */
void HybridReachability::run(string target)
{
	int targetID = getIDForMode(target);
	if(targetID<0){
		printf("mode: %s is not declared\n",target.c_str());
		exit(1);
	}
	// normalize the candidate vectors
	for(int i=0; i<aggregationTemplate_candidates.size(); ++i)
	...
	if(system.DFS(flowpipesCompo, domains, modeIDs, traceNodes, traceTree, integrationSchemes, step, miniStep, time, orderType, orders, maxOrders, globalMaxOrder, bAdaptiveSteps, bAdaptiveOrders,
			maxJumps, precondition, estimation, aggregationType, aggregationTemplate_candidates, default_aggregation_template, weightTab,
			linear_auto, template_auto, bPrint, stateVarNames, modeNames, tmVarNames,targetID,-1,0))
		printf("The target is reachable\n");
	else
		printf("The target is not reachable\n");
}
/* DFS traversing */
bool HybridSystem::DFS(list<list<TaylorModelVec> > & resultsCompo, list<list<vector<Interval> > > & domains, list<int> & modeIDs,
			list<TreeNode *> & traceNodes, TreeNode * & traceTree, const vector<int> & integrationSchemes, const double step, const double miniStep,const double time, const int orderType, const vector<int> & orders, const vector<int> & maxOrders, const int globalMaxOrder,
			const bool bAdaptiveSteps, const bool bAdaptiveOrders,const int maxJumps,const int precondition, const vector<Interval> & estimation,const vector<vector<int> > & aggregType, const vector<vector<vector<RowVector> > > aggregationTemplate_candidates, const vector<RowVector> default_aggregation_template,const vector<vector<Matrix> > & weightTab, const vector<vector<vector<bool> > > & linear_auto, const vector<vector<vector<RowVector> > > & template_auto,
			const bool bPrint, const vector<string> & stateVarNames, const vector<string> & modeNames, const vector<string> & tmVarNames,
	int target,int cur_mode,int cur_step)

{
	if(cur_mode<0)
		cur_mode = initialMode;
	//printf("visit %s\n",modeNames[cur_mode].c_str());
	path.push_back(cur_mode);	
	if(cur_mode == target){
		print_path(modeNames,path);
		if(reach_path(resultsCompo, domains, modeIDs, traceNodes, traceTree, integrationSchemes, step, miniStep, time, orderType, orders, maxOrders, globalMaxOrder, bAdaptiveSteps, bAdaptiveOrders,path, precondition, estimation, aggregType, aggregationTemplate_candidates, default_aggregation_template, weightTab,
			linear_auto, template_auto, bPrint, stateVarNames, modeNames, tmVarNames)){
			return true;
		}

	}
	if(cur_step==maxJumps){
		//printf("touch the bound\n");		
		return false;
	}
	for(int i=0; i<transitions[cur_mode].size(); ++i){
		int next_mode=transitions[cur_mode][i].targetID;
		if( DFS(resultsCompo, domains, modeIDs, traceNodes, traceTree, integrationSchemes, step, miniStep, time, orderType, orders, maxOrders, globalMaxOrder, bAdaptiveSteps, bAdaptiveOrders, maxJumps,precondition, estimation, aggregType, aggregationTemplate_candidates, 
default_aggregation_template, weightTab, linear_auto, template_auto, bPrint, stateVarNames, modeNames, tmVarNames,target, next_mode, cur_step+1))
			return true;
		path.pop_back();
...
void HybridSystem::print_path(const vector<string> & modeNames,vector<int> path1){
	printf("Checking path:");	
	for(unsigned i=0;i<path1.size();i++){
		printf("%s ",modeNames[path1[i]].c_str());
	}
	printf("\n");
}
...
void HybridReachability::check_path()
{	
	...
	compute_power_4(globalMaxOrder+1);
	compute_double_factorial(2*globalMaxOrder);
	if(system.reach_path(flowpipesCompo, domains, modeIDs, traceNodes, traceTree, integrationSchemes, step, miniStep, time, orderType, orders, maxOrders, globalMaxOrder, bAdaptiveSteps, bAdaptiveOrders,
			path, precondition, estimation, aggregationType, aggregationTemplate_candidates, default_aggregation_template, weightTab,
			linear_auto, template_auto, bPrint, stateVarNames, modeNames, tmVarNames))
		printf("The path is reachable\n");
	else
		printf("The path is not reachable\n");
}
...
// add by xiedingbao
bool HybridReachability::setPath(string text){
	stringstream ss(text);
	string name;	
	while(getline(ss, name, '@')){
		int ID = getIDForMode(name);
		if(ID<0){
			printf("mode: %s is not declared\n",name.c_str());
			return false;
		}
