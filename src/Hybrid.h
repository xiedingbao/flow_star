...
private:
	...
	Flowpipe initialSet;
	vector<int> path;
	void print_path(const vector<string> & modeNames,vector<int> path);
public:
	HybridSystem();
...
// added by Dingbao Xie
	bool reach_path(list<list<TaylorModelVec> > & resultsCompo, list<list<vector<Interval> > > & domains, list<int> & modeIDs,
			list<TreeNode *> & traceNodes, TreeNode * & traceTree, const vector<int> & integrationSchemes, const double step, const double miniStep,
			const double time, const int orderType, const vector<int> & orders, const vector<int> & maxOrders, const int globalMaxOrder,
			...
			const bool bPrint, const vector<string> & stateVarNames, const vector<string> & modeNames, const vector<string> & tmVarNames) const;
//added by Dingbao Xie
bool DFS(list<list<TaylorModelVec> > & resultsCompo, list<list<vector<Interval> > > & domains, list<int> & modeIDs,
			...
			const bool bAdaptiveSteps, const bool bAdaptiveOrders,const int maxJumps, const int precondition, const vector<Interval> & estimation,const vector<vector<int> > & aggregType, const vector<vector<vector<RowVector> > > aggregationTemplate_candidates, const vector<RowVector> default_aggregation_template,
			const vector<vector<Matrix> > & weightTab, const vector<vector<vector<bool> > > & linear_auto, const vector<vector<vector<RowVector> > > & template_auto,
...
public:
	...
	void run();
	void run(string target);//added by Dingbao Xie
	void check_path();	//added by Dingbao Xie
